...
/**
 * Returns the next value from the given LinkedListIterator, without
 * incrementing it to point to the next element
 */
void* LinkedListIterator_peek(LinkedListIterator this) {
    if (this->next == NULL) {
        return NULL;
    } else {
        void *data = this->next->data;
...
int get_LinkedList_size(const LinkedList linked_list) {
